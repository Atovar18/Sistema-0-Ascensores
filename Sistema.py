...
B_t= ((B_i * 2 * 30) + (15*3)) #Valor de personas por piso por el factor de ocupacion, sumando los sotanos multiplicado por el numero de personas por sotano.
print  (B_t , "personas estimadas a utulizar los ascensores.")
P_v = (32//pn + 0.7*pn + 0.5) #Pasageros por viaje.
...
B = ((n*Ar)/10) #n_a va a variar segun sea el piso maximo a donde llegue las ascensores.
...
if Q > V_n: #Condiciones para poder calcular el tiempo de viaje.
    TVC = ((((2*H_a)/(V_n)))-((H_s/V_n))+((2*V_n)/(Q))+(((2*H_s)/((n_p*((((H_s*Q)/n_p)**0.5)))))*(n_p-1)) + (T_1*(n_p+1) + (T_2)*(P_v) ))
    print (TVC , "s de viaje sobre las plantas")
else: 
    TVC = ((2*H_a)/(V_n))+(((V_n/Q)+T_1)*(n_p+1))-((H_s/(n_p*(V_n)))+((T_2)*(P_v)))
